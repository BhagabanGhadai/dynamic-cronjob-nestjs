...
export class TaskHelper {
    ...
    loadTask(task: TaskDocument) {
        if (task.status === ECurrentStatus.inactive || task.status === ECurrentStatus.delete) {
        ...
        if (task.taskType === ETaskType.interval) {
            return this.schedulerRegistry.getInterval(task.name);
        }
        if (task.taskType === ETaskType.timeout) {
            return this.schedulerRegistry.getTimeout(task.name);
        }
    }
    ...
    startCronJob(task: TaskDocument) {
        const newJob = new CronJob(task.cronExpression, () => {
    ...
    startIntervalJob(task: TaskDocument) {
        const interval = setInterval(() => {
            this.logger.log("Interval job started");
        }, task.intervalInMs)
    ...
    startTimeoutJob(task: TaskDocument) {
        const timeout = setTimeout(() => {
            this.logger.log("Timeout job started");
        }, task.timeoutInMs)
