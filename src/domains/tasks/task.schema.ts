...
const intervalTaskSchema = baseTaskSchema.extend({
  ...
  intervalInMs: z
    .number({ message: 'intervalInMs is required and must be a number' })
    .positive('intervalInMs must be a positive number'),
});

const timeoutTaskSchema = baseTaskSchema.extend({
  taskType: z.literal(ETaskType.timeout),
  timeoutInMs: z
    .number({ message: 'timeoutInMs is required and must be a number' })
    .positive('timeoutInMs must be a positive number'),
});
...
export const updateTaskSchema = z.object({
  ...
  cronExpression: z.optional(
    z.string().refine((val) => TaskHelper.isCronExpression(val), {
      message: 'Invalid cron expression',
    }),
  ),
  timeZone: z.optional(z.nativeEnum(ETimeZone)),
  intervalInMs: z.optional(
    z
      .number({ message: 'intervalInMs is required and must be a number' })
      .positive('intervalInMs must be a positive number'),
  ),
  timeoutInMs: z.optional(
    z
      .number({ message: 'timeoutInMs is required and must be a number' })
      .positive('timeoutInMs must be a positive number'),
  ),
